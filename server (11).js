...
function autoBackup() {
  try {
    const d = new Date();
    const tag = d.getFullYear() + ('0'+(d.getMonth()+1)).slice(-2) + ('0'+d.getDate()).slice(-2);
    const backupFile = path.join(DATA_DIR, 'products_backup_' + tag + '.json');
    if (!fs.existsSync(backupFile)) {
      fs.copyFileSync(PRODUCTS_FILE, backupFile);
      // 30일 이전 백업 자동 삭제
      fs.readdirSync(DATA_DIR).forEach(f => {
        if (f.startsWith('products_backup_') && f.endsWith('.json') && f !== path.basename(backupFile)) {
          const fDate = f.replace('products_backup_','').replace('.json','');
          if (fDate < tag.slice(0,-2) + '00') try { fs.unlinkSync(path.join(DATA_DIR, f)); } catch(e) {}
        }
      });
